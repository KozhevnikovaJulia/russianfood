        ...
        {ingredientFields.map((field, index) => (
          <div key={field.id} className='flex gap-2 items-center'>
            ...
            <Input
              ...
              className='w-[100px]'
              onChange={e => handleIngredientChange(field.id, 'quantity', e.target.value ? parseFloat(e.target.value) : null)}
              validate={value => (!value || parseFloat(value) <= 0 ? 'Количество должно быть больше 0' : null)}
            />
            {ingredientFields.length > 1 && (
              <Button color='danger' variant='light' onPress={() => handleRemoveIngredientField(field.id)} className='w-[50px]'>
                -
              </Button>
            )}
          </div>
